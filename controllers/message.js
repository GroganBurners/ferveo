...
module.exports = class MessageController {
  ...
  sendSMS(sms) {
    ...
    return rp("https://www.my-cool-sms.com/api-socket.php", {
      json: true,
      body: data
    }).then(resp => {
  ...
  sendTestEmail(body) {
    const mailOptions = {
      from:
        '"' + config.mail.from.name + '" <' + config.mail.from.address + ">", // sender address
  ...
  sendEmail(email) {
    ...
    return transporter.sendMail(email).then(() => {
      return { message: "Message sent" };
    });
    /* .catch((err) => {
            return new Error('Error creating message: ' + err)
  ...
  routeAPI() {
    ...
    router.post("/email", (req, res) => {
      this.sendEmail(req.body).then(ok(res)).then(null, fail(res));
    });

    router.post("/sms", (req, res) => {
      this.sendSMS(req.body).then(ok(res)).then(null, fail(res));
    });
