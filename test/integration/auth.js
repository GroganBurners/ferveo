describe("Auth logged out", function() {
  ...
  it("Account page logged out redirects", done => {
    chai.request(server).get("/auth/account").end(function(err, res) {
      should.not.exist(err);
  ...
  it("google endpoint redirects", function(done) {
    chai.request(server).get("/auth/google").end(function(err, res) {
      should.exist(err);
  ...
  it("it should end the session and show login", function(done) {
    chai.request(server).get("/auth/logout").end(function(err, res) {
      should.not.exist(err);
...
describe("GET /auth/google/callback", () => {
  beforeEach(function() {
    nock("https://accounts.google.com").post("/o/oauth2/token").reply(200, {
  ...
  xit("Login works and shows account", done => {
    ...
    try {
      agent
        .get(
          "/auth/google/callback?code=xxxxxxxx&authuser=0&session_state=xxxxxxxx&prompt=consent"
        )
        .then(function(res) {
          res.should.have.status(200);
